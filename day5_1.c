...
uint64_t find_answer(uint_list_list_t* maps, uint64_t level, uint64_t num) {
    ...
    return find_answer(maps + 1, level + 1, num);
}
...
int main() {
    ...
    for (int i = 0; i < seeds->size; i++) {
        uint64_t loc = find_answer(maps, 0, seeds->list[i]);
        printf("%lu\n", loc);
        min = (min > loc) ? loc : min;
    }
    printf("\n\n%lu\n", min);
